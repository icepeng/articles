...
??가 &&보다 더 안전하다. ??는 nullish 체크이기 때문에 `null`, `undefined`만 우측 값을 평가하는 반면, ||는 falsy 체크이기 때문에 `0`, `''`, `false` 까지도 범위에 포함하는 문제가 있다. 만약 `null`, `undefined`만을 체크하는 목적으로 사용했다면 예상치 못한 버그가 발생할 수 있다. 이는 Optional Chaining에도 동일하게 적용된다.
